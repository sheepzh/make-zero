...
function translate(obj: any, parentKey = '') {
    const result = {}
...
class GenerateLocaleForChrome implements webpack.WebpackPluginInstance {
    ...
    async apply(compiler: webpack.Compiler) {
        ...
        const outFilePath = path.resolve(outputPath, `${this.outputFileName}.js`)
        compiler.hooks.done.tap('GenerateLocaleForChromePlugin', () => {
            ...
            for (const localeName in messages) {
                ...
                const dir = path.resolve(outputPath, '_locales', localeName)
                fs.mkdirSync(dir, { recursive: true })
                fs.writeFileSync(path.resolve(dir, 'messages.json'), JSON.stringify(chromeMessage))
            }
